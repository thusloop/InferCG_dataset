...
def read_pypi_data():
    with open('ConstructKB/Dependency_Level/pypi_packages_normal.txt','r') as f:
...
def IsPyPIlibrary(pkg,pypi_server="https://pypi.python.org/pypi/",proxy=None):
    if len(pkg) == 0 or pkg == '.' or pkg == '*': #empty
        return False
    ...
    for item in alldeps:
        if item['status'] != 'file':
            ...
            if IsPyPIlibrary(dep_name):
                final_deps.append({'dep':dep_name,'version':version,'filepath':item['filepath'],'type':'*','condition':item['condition']+'@'+extra_info})
